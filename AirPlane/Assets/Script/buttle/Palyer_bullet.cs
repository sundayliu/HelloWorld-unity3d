	...
	void Update () {
		...
		case bullettype.one:
			if (Bulletshoot) {
				...
				bullet.transform.FindChild ("butlle").GetComponent<buttle> ().player = true;
				bullet.transform.FindChild ("butlle").GetComponent<buttle> ().buttlePower=bulletpower;
				Bulletshoot=false;
			}
			break;
		case bullettype.two:
			if (Bulletshoot) {
				...
				bullet.transform.FindChild ("butlle").GetComponent<buttle> ().player = true;
				bullet.transform.FindChild ("butlle").GetComponent<angle_buttle> ().agnel=agnel;
				bullet.transform.FindChild ("butlle").GetComponent<angle_buttle> ().L=L;
				Bulletshoot=false;
			}
			break;
				}
